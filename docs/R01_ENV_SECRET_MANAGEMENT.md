...
CI runs `scripts/validate_env_secret_management.py` plus repository security hygiene. The R01 validator checks client/static files for server-secret identifiers/material, verifies the committed environment template contains placeholders rather than credential values, and verifies the explicit public-key classification.
