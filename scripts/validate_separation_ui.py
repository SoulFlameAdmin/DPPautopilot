...
def main() -> None:
    ...
    model_paths = set(re.findall(r'data-model-path="([^"]+)"', dom))
    item_paths = set(re.findall(r'data-item-path="([^"]+)"', dom))
    ...
    require(model_paths.isdisjoint(item_paths), "model/item path sets overlap")
