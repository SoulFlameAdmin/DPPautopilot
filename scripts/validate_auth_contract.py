...
require("function refreshDelayMs(expiresIn)" in html,"M01 refresh scheduler helper missing")
require("Math.min(60000,Math.max(1000,Math.floor(ttlMs*0.1)))" in html,"M01 refresh scheduler must use bounded proportional lead time")
require("return Math.max(1000,ttlMs-leadMs)" in html,"M01 refresh scheduler must schedule from TTL minus bounded lead")
require("Math.max(1000,(expiresIn-60)*1000)" not in html,"M01 refresh scheduler must not use fixed 60-second lead that can create 1-second refresh loops")
require("clearTimeout(refreshTimer)" in html,"M01 session replacement/signout must clear stale refresh timers")
require("AUTH_REQUEST_TIMEOUT_MS=15000" in html,"M01 auth requests must have a deterministic timeout")
require("new AbortController()" in html and "signal:controller.signal" in html,"M01 auth requests must be abortable")
require("clearTimeout(timeout)" in html,"M01 auth request timeout must be cleared after completion")
require("Authentication request timed out. Please try again." in html,"M01 auth timeout must expose a stable retryable error")
require("if(!cfg) throw new Error('auth client is not ready')" in html,"M01 auth calls must fail closed before client initialization")
