...
require("setSession(d);out('Session refreshed successfully.','ok')" in html,"M01 refresh flow must rotate the in-memory session through setSession")
require("if(!refreshToken)" in html,"M01 refresh flow must fail closed when no refresh token is available")
