-- R07/M15 privacy hardening: reject obvious credential-bearing keys from registry request payloads.
-- This is a technical minimization guard only; it does not define the final provider/legal payload allowlist.
...
begin
  ...
  if v_type='object' then
    ...
    loop
      ...
      if public.dpp_json_has_credential_key(v_child) then
        return true;
      end if;
    end loop;
    return false;
...
comment on function public.dpp_json_has_credential_key(jsonb) is
  'R07 technical minimization helper: recursively detects obvious credential-bearing JSON keys before registry payload persistence.';
comment on function public.dpp_reject_registry_payload_credentials() is
  'R07/M15 fail-closed registry request minimization guard. It does not define the final provider/legal payload allowlist.';
